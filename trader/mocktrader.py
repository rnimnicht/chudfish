...
def run_it_up(market):
    ...
    if not kalshi.yes_asks or not kalshi.no_asks or not poly.yes_asks or not poly.no_asks:
    ...
    if combo1 < 1.0:
        arb_collection.insert_one({
            ...
            'min_volume': min(poly.no_asks[poly_no_max] * poly_no_max, kalshi.yes_asks[kalshi_yes_max] * poly_no_max),
            'timestamp': now,
        })
        print(f"ARB kalshi_yes+poly_no={combo1:.4f} for {market}")
    ...
    if combo2 < 1.0:
        arb_collection.insert_one({
            ...
            'kalshi_no_size': kalshi.no_asks[kalshi_no_max],
            'poly_yes_price': poly_yes_max,
            'poly_yes_size': poly.yes_asks[poly_yes_max],
            'combined_price': combo2,
            'timestamp': now,
        })
        print(f"ARB kalshi_no+poly_yes={combo2:.4f} for {market}")
